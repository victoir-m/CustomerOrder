...
{
    public abstract class CustomerOrdersTestBase : AbpIntegratedTestBase<CustomerOrdersTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected CustomerOrdersTestBase()
        {
        ...
        protected void UsingDbContext(Action<CustomerOrdersDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<CustomerOrdersDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CustomerOrdersDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CustomerOrdersDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CustomerOrdersDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CustomerOrdersDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<CustomerOrdersDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CustomerOrdersDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CustomerOrdersDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CustomerOrdersDbContext, Task<T>> func)
        {
            T result;
